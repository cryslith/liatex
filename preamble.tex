\usepackage[shortlabels]{enumitem}
\usepackage{amsmath,amsfonts,amssymb}
\usepackage{latexsym}
\usepackage{gensymb}
\usepackage{mathtools}
\usepackage{xifthen}
\usepackage[dvipsnames]{xcolor}
\usepackage{tcolorbox}

\usepackage{amsthm}

\usepackage{tikz}

% nice fonts
\usepackage{libertine}
\usepackage[libertine,vvarbb]{newtxmath}
\usepackage[scaled=0.9]{zi4}
\DeclareMathAlphabet{\mathcal}{OMS}{cmsy}{m}{n}

% spaces between paragraphs instead of indent
\usepackage{parskip}
% avoid adding the extra space around proofs
\usepackage{etoolbox}
\BeforeBeginEnvironment{proof}{\vspace*{-\parskip}}
\AfterEndEnvironment{proof}{\vspace*{-\parskip}}

% nice links
\usepackage[colorlinks, linkcolor=purple, citecolor=blue!75!black, urlcolor=purple]{hyperref}

% replacement for cleveref
\usepackage{zref-clever}
\zcsetup{cap=true}
\newcommand{\cref}[1]{\zcref{#1}}

\newcounter{theorem}

\newcommand{\mkthm}[2]{%
  \newtheorem{#1}[theorem]{#2}%
  \AddToHook{env/#1/begin}{\zcsetup{countertype={theorem=#1}}}%
  \newtcolorbox[use counter=theorem]{#1box}[2][]{
    label type=#1,
    title=\textbf{#2~\thetcbcounter}\ifthenelse{\isempty{##2}}{}{ ({##2})}\textbf{.},
    colback=WildStrawberry!5,
    colframe=WildStrawberry!30,
    coltitle=black,
    ##1
  }%
  \newtheorem*{#1*}{#2}%
  \newtcolorbox{#1boxun}[2][]{
    title=\textbf{#2}\ifthenelse{\isempty{##2}}{}{ ({##2})}\textbf{.},
    colback=WildStrawberry!5,
    colframe=WildStrawberry!30,
    coltitle=black,
    ##1
  }
}

\mkthm{theorem}{Theorem}
\mkthm{lemma}{Lemma}
\mkthm{proposition}{Proposition}
\mkthm{corollary}{Corollary}
\theoremstyle{definition}
\mkthm{definition}{Definition}
\mkthm{example}{Example}
\mkthm{remark}{Remark}
\mkthm{fact}{Fact}
\mkthm{claim}{Claim}

% left margin notes
\usepackage{marginnote}
\reversemarginpar
\renewcommand*{\marginfont}{\scriptsize\sffamily}

\newcommand{\xxx}[1]{{\normalmarginpar\marginnote{\textcolor{red}{\large xxx}}} \textbf{[xxx }{#1}\textbf{]}}

\makeatletter
% superimpose two math symbols
\newcommand{\superimpose}[2]{{%
  \ooalign{%
    \hfil$\m@th#1\@firstoftwo#2$\hfil\cr
    \hfil$\m@th#1\@secondoftwo#2$\hfil\cr
  }%
}}
\makeatother

\newcommand*{\tensor}{\otimes}
\newcommand*{\mat}[1]{\begin{bmatrix}#1\end{bmatrix}}
\newcommand*{\tp}[0]{\mathsf{T}}
\renewcommand*{\iff}[0]{\leftrightarrow}
\renewcommand*{\emptyset}[0]{\varnothing}
\newcommand{\dotcup}[0]{\mathbin{\mathpalette\superimpose{{\cdot}{\cup}}}}
\DeclareMathOperator*{\bigdotcup}{\mathpalette\superimpose{{\cdot}{\bigcup}}}
\newcommand*{\poly}{\mathsf{poly}}
\newcommand*{\E}{\mathbf{E}}
\DeclareMathOperator*{\Var}{Var}
\DeclareMathOperator*{\Cov}{Cov}
\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\esssup}{ess\,sup}
\DeclareMathOperator*{\essinf}{ess\,inf}
\DeclareMathOperator*{\tr}{tr}
\DeclareMathOperator*{\dom}{dom}
\DeclareMathOperator*{\cod}{cod}
\DeclarePairedDelimiter{\norm}{\lVert}{\rVert}
\DeclarePairedDelimiter{\floor}{\lfloor}{\rfloor}
\DeclarePairedDelimiter{\ceil}{\lceil}{\rceil}
\newcommand*{\C}{\mathbb{C}}
\newcommand*{\F}{\mathbb{F}}
\newcommand*{\N}{\mathbb{N}}
\newcommand*{\Q}{\mathbb{Q}}
\newcommand*{\R}{\mathbb{R}}
\newcommand*{\Z}{\mathbb{Z}}

\def\[#1\]{\begin{align*}#1\end{align*}}
\newcommand*{\aln}[1]{\begin{aligned}#1\end{aligned}}
\newcommand*{\pheq}{\mathrel{\phantom{=}}} % invisible = for align mode
\newcommand*{\mmid}{\mathrel{}\middle|\mathrel{}}

\newcommand*{\defn}[1]{\textbf{\textit{\boldmath #1}}}

\newcommand*{\eqdef}[0]{\mathrel{\begin{tikzpicture}[scale=0.06,baseline={(e.south)}]
      \draw (0, 0) -- (1, {-sqrt(3)}) -- (-1, {-sqrt(3)}) -- cycle;
      \node[anchor=base,inner sep=0pt] (e) at (0, -5) {$=$};
\end{tikzpicture}}}

\newcommand{\hidden}[3][]{\ifthenelse{\isempty{#1}}{\emph{{#2} hidden.}}{#3}}
